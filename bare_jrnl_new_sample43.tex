...
task-coupled.
\item It develops a two-stage LAM-traffic-recognition case study. Before an
operator--OTT agreement, passive recognition uses encrypted uplink/downlink
traffic dynamics to distinguish generative, agentic, embodied, and legacy
mobile services; after agreement, active recognition uses authenticated
service, transaction, deadline, and reliability descriptors for enforceable
QoS mapping.
\end{itemize}
...
After an operator--OTT agreement, let $c\in\{G,A,E,U\}$ denote the declared
generative, agentic, embodied, or unknown class. A signed metadata descriptor
$m$ contains an opaque service ID, session/chunk identifier, timestamp,
deadline bin, reliability tier, and token- or byte-count bin. Traffic features
$x$ remain available for consistency checking. Active recognition is then a
cost-sensitive decision
\begin{equation}
\hat c^{\rm act}=\arg\min_{j\in\{G,A,E,U\}}
\sum_i C_{ij}\Pr(c=i\mid x,m),
\label{eq:recognition}
\end{equation}
where $C_{ij}$ distinguishes false promotion, which wastes reserved resources,
from false demotion, which risks an admitted SLO. A traffic-shape mismatch can
trigger re-authentication or re-admission, but cannot silently override the
signed class.
...
is absent, invalid, or unsupported, the flow falls back to passive recognition
and ordinary connectivity rather than receiving an unverified high-priority
class. Thus the active stage enables an auditable commercial contract without
requiring the operator to inspect model inputs, private memory, or physical
task semantics; its experimental validation is left to a later deployment
study, as specified in the two-stage design.
