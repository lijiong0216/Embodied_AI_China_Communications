...
planning quality, not merely reconstruction, since a small perception error
...
\begin{itemize}
    \item First, evaluation moves from \emph{relative content accuracy}, to \emph{practical process success}, to \emph{physical closed-loop performance}. 
    \item Second, interaction frequency and coupling increase: a prompt is weakly coupled to the next request, an agent trajectory is logically coupled across steps, and an embodied trajectory is both logically and dynamically coupled through the environment. 
    \item Third, the consequence of failure changes from a correctable response to an external state change and finally to a potentially irreversible physical event. 
    \item Fourth, the quantitative workload grows through longer contexts, more model invocations, and denser multimodal streams, whereas the dominant qualitative model concern changes from generation fidelity, to trajectory correctness and memory, and then to grounded control and safety.
\end{itemize}
