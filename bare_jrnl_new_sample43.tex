...
generalization \cite{metaworld}; RT-2 uses thousands of real-robot trials to
test seen and unseen skills and semantic generalization \cite{rt2}; and LIBERO
measures transfer and robustness across lifelong manipulation task suites
\cite{libero}. VLA evaluations accordingly report task success, trajectory
efficiency, collision rate, action error, robustness to scene perturbation, and
sim-to-real degradation. World models are judged by predictive consistency and
planning quality, not merely reconstruction. Because a small perception error
is harmless in one state and catastrophic near a contact transition, average
token distortion alone is inadequate.

Safe reinforcement learning formalizes episodic return together with safety
constraints or risk penalties \cite{saferl}. Following that established
evaluation pattern, a benchmark-defined control cost can be written as
\begin{equation}
...
constraint violation. This is not proposed as a universal new score: each
benchmark must publish its horizon, weights, and safety definition and should
also report the component metrics separately.
...
and inference cost. This accuracy--responsiveness tension is difficult, but the
streams are not irreducible. Visual tokens contain \emph{spatial} redundancy
because only a subset describes task-relevant objects, and action tokens contain
\emph{temporal} redundancy during stable motion while becoming highly sensitive
near transitions. FlashVLA exploits visual-token pruning and action reuse,
reporting substantial compute and latency savings with a small success-rate
reduction on LIBERO \cite{flashvla}; algorithm--architecture co-design provides
another route to real-time embodied inference \cite{dadu}. These results
motivate exploiting redundancy, while also showing why small corrective actions
near an unstable equilibrium cannot be removed indiscriminately.
...
Table~\ref{tab:modeltaxonomy} reveals four recurring development laws. First,
evaluation moves
...
state change and finally to a potentially irreversible physical event. Fourth,
the quantitative workload grows through longer contexts, more model invocations,
and denser multimodal streams, whereas the dominant qualitative model concern
changes from generation fidelity, to trajectory correctness and memory, and
then to grounded control and safety.

The increasing structure of model inputs and outputs creates an opportunity as
well as a burden. Generative tokens mainly expose statistical redundancy;
agentic trajectories add repeated context and workflow redundancy; embodied
streams add constraints induced by geometry, kinematics, and task relations.
Selective computation, caching, pruning, and action reuse can exploit these
regularities, but their errors must be evaluated at the content, task, or
physical-episode level appropriate to the stage.
...
adding state, timing, interaction, and physical grounding.

\subsection{Benchmark Design Across Stages}
A benchmark supports a valid cross-stage comparison only when its execution
...
Second, stressors must be injected at the model interface under study.
...
Third, the benchmark must propagate a stressor to the terminal metric. For a
...
state estimation, control, and safety. Table~\ref{tab:qos} gives a reproducible
scenario envelope that instantiates the radar-style comparison motivating this
survey. The entries are design anchors, not standardized 6G requirements or
simultaneous maxima.
...
Two cautions are essential. First, multiplying every maximum in one row by
every maximum in another creates an impossible workload; duty factors and
statistical multiplexing must be declared. Second, a ``token'' is not a fixed
number of bits across text, images, video, latent states, or actions. Token
rate therefore complements rather than replaces bit rate. The assumptions in
Table~\ref{tab:qos} make sensitivity analysis possible: changing $r_0$,
$N_{\rm par}$, $L_{\rm state}$, $a$, or trajectory length $H$ immediately
produces a new deployment-specific envelope.

\subsection{Generative Stage: Fidelity and Cost under Elastic Interaction}
Generative serving decomposes delay into request admission, prefill, and
decode. Operational metrics include time to first token (TTFT), time per output
token (TPOT), completion time, token/bit goodput, and deadline attainment.
Multimodal diversity requires scalable acquisition and fusion, while generation
quality limits aggressive token removal: adaptive video tokenization, for
example, explicitly trades sequence length against reconstruction and
generation quality \cite{evatok}. Computation is strongly asymmetric between
prefill and autoregressive decode, so batching, cache placement, and the
device--edge--cloud split can dominate radio time \cite{edgeLLMsurvey}.

Let $q_i$ be response quality, $T_i^{\rm f}$ TTFT, $r_i^{\rm tok}$ useful
output rate, and $e_i$ the communication--compute--storage cost. A generic
service utility is
\begin{equation}
...
\end{equation}
where $[z]^+=\max(z,0)$. Interactive text emphasizes TTFT/TPOT, offline media
emphasizes completion energy, and high-fidelity generation emphasizes semantic
quality and sustained rate. A token-responsive cost-effectiveness metric is
therefore more informative than raw throughput or bits/J alone \cite{tree}.

\subsection{Agentic Stage: Long-Horizon Goodput, Memory, and Retry}
An agentic model autonomously plans, decomposes, reflects, retrieves memory,
and invokes tools \cite{agenticsystems}. Every model response is an
intermediate event, so useful network goodput is the fraction of transmitted
state that contributes to a correctly completed task. End-to-end job time,
task success, recovery probability, context freshness, unnecessary calls, and
cost per completed task qualify the same underlying latency, reliability,
rate, storage, and energy metrics. Branching, timeouts, debate, and retries
produce long tails and repeated token generation/transmission; persistent
personalization further turns memory movement and cache affinity into network
requirements \cite{agenticenergy}.

A packet may arrive correctly while a stale observation, duplicate
non-idempotent tool call, incompatible schema, or poisoned memory causes task
failure. Conversely, a lost low-value rationale need not be retransmitted when
a verified tool result is intact. A useful transaction-level SLO is
\[
{\cal S}_{\rm ag}=(P_{\rm task},T_{95}^{\rm job},A_{\rm mem},
C_{\rm job},P_{\rm unsafe}),
\]
where $A_{\rm mem}$ is the age of relevant memory and $C_{\rm job}$ includes
communication, inference, storage, and tool access. Network-aware model-context
platforms expose capability and path state \cite{netmcp}; representation and
cache exchange can reduce repeated text and prefill traffic
\cite{qkvcomm,kvcomm}, but admission, idempotence, rollback, and audit still
apply to the complete task graph.
...
learning,'' \emph{J. Mach. Learn. Res.}, vol. 16, no. 42, pp. 1437--1480,
2015.
...
\bibitem{tree}
T. Yu \etal, ``TREE: Token-responsive energy efficiency framework for green
AI-integrated 6G networks,'' \emph{IEEE Wireless Commun.}, early access, 2026,
doi: 10.1109/MWC.2025.3641529.

\bibitem{agenticsystems}
F. Bousetouane, ``Agentic systems: A guide to transforming industries with
vertical AI agents,'' arXiv:2501.00881, Jan. 2025.

\bibitem{agenticenergy}
X. Chen, H. Yu, W. Ni, D. Niyato, R. Zhang, X. Wang, S. Zhang, and S. Xu,
``Networking-aware energy efficiency in agentic AI inference: A survey,''
arXiv:2604.07857, Apr. 2026.
